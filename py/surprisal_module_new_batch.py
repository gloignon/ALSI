...
def set_model(model_name, mode, force_device=None, use_fast=False, trust_remote_code=True):
...
#     global _model, _tokenizer, _mode, _device
#     
#     # print the global parameters to console for debugging
#     # print(f"Mode: {_mode}")
# 
#     # Extract data
#     left_ids_list = []
...
#     
#     for item in batch:
#         # Ensure all inputs are lists of integers
#         left_ids = item.get("left_ids", [])
#         target_ids = item.get("target_ids", [])
#         right_ids = item.get("right_ids", [])
#         
#         # Convert to list if not already a list
#         left_ids = left_ids if isinstance(left_ids, list) else [left_ids]
#         target_ids = target_ids if isinstance(target_ids, list) else [target_ids]
#         right_ids = right_ids if isinstance(right_ids, list) else [right_ids]
#         
#         # Append to our batch lists
#         left_ids_list.append(left_ids)
#         target_ids_list.append(target_ids)
#         right_ids_list.append(right_ids)
#     
#     # Process based on mode
#     if _mode == "ar":
#         results = _batch_surprisal_ar(left_ids_list, target_ids_list, right_ids_list)
#     elif _mode == "mlm":
#         results = _batch_surprisal_mlm(left_ids_list, target_ids_list, right_ids_list)
#     elif _mode == "pseudo_ar":
#         results = _batch_surprisal_pseudo_ar(left_ids_list, target_ids_list, right_ids_list)
#     else:
    ...
    if _mode == "ar":
        ...
        for i in range(len(batch)):
            input_ids = torch.tensor(
                batch_left_ids[i] + batch_target_ids[i] + batch_right_ids[i],
                dtype=torch.long, device=_device
            ).unsqueeze(0)
            target_start = len(batch_left_ids[i])
            target_end = target_start + len(batch_target_ids[i])
            target_tokens = _tokenizer.convert_ids_to_tokens(batch_target_ids[i])
            out = _surprisal_ar(input_ids, target_start, target_end, target_tokens)
            results.append(out)
        return results
