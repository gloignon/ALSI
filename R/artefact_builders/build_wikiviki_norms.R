...
# Parsed corpus is not cached; each run re-parses from the TSV.
#
# References:
...
#   Rscript R/artefact_builders/build_wikiviki_norms.R
...
parse_texts <- function(which) {
  ...
  parse_text(dt_txt, n_cores = n_cores, parser = "none")
}
...
dt_hard <- parse_texts("hard")
dt_easy <- parse_texts("easy")
